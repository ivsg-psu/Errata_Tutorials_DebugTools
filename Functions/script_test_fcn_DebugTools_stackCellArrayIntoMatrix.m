%% script_test_fcn_DebugTools_stackCellArrayIntoMatrix
...
%% DEMO case: Simple demo

figNum = 10001;
titleString = sprintf('DEMO case: Simple demo');
fprintf(1,'Figure %.0f: %s\n',figNum, titleString);
figure(figNum); clf;

clear cellArrayToStack
cellArrayToStack = cell(2,1);
cellArrayToStack{1,1} = [1 2 3; 4 5 6];
cellArrayToStack{2,1} = [7 8 9];
...
expectedMatrix = [
	...
	7     8     9
	NaN   NaN   NaN];
assert(isequaln(stackedMatrix,expectedMatrix));
...
% Call the function
stackedMatrix = fcn_DebugTools_stackCellArrayIntoMatrix(cellArrayToStack, ([]));
...
% Call the function
stackedMatrix = fcn_DebugTools_stackCellArrayIntoMatrix(cellArrayToStack, (-1));
