function cellString = fcn_DebugTools_convertVariableToCellString(inputVariable, varargin)
...
% FORMAT:
%
%      cellString = fcn_DebugTools_convertVariableToCellString(inputVariable)
%
% INPUTS:
%
%      inputVariable: a numeric, character, or cell array
%
%      (OPTIONAL INPUTS)
%
...
if iscell(inputVariable)
    % OLD:     if length(inputVariable)==1
    if isscalar(inputVariable)
        if ischar(inputVariable{1})            
            cellString = inputVariable{1};
        elseif isnumeric(inputVariable{1})
            cellString = fcn_DebugTools_number2string(inputVariable{1});
        else
        ...
        for ith_cell = 1:length(inputVariable)-1
            result = fcn_DebugTools_convertVariableToCellString(inputVariable{ith_cell});
            temp = cat(2,temp,char(result),', ');
        end
        result = fcn_DebugTools_convertVariableToCellString(inputVariable{length(inputVariable)});
        temp = cat(2,temp,char(result)); % Last concatenation does not have space or comma at end
        cellString = {temp};
    end
elseif isnumeric(inputVariable)
    cellString = {fcn_DebugTools_number2string(inputVariable)};
elseif ischar(inputVariable)
    cellString = {inputVariable};
else
